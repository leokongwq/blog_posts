...
若您在集群模式下的 Event Bus 中撤销处理器，则同样会花费一些时间在节点中传播。若您想在完成后收到通知，可以使用`unregister(handler)` 方法注册处理器：
...
`EventBusOptions`还允许您指定 `Event Bus` 是否运行在集群模式下，以及它的主机信息和端口。您可使用`setClustered`、`getClusterHost`和`getClusterPort` 方法来设置。
